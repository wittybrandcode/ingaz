...
router.put('/types/:id', authenticate, requireManager, validate(updateWarningTypeSchema), tryCatch(async (req, res) => {
...
router.put('/:id/clear', authenticate, requireManager, tryCatch(async (req, res) => {
  const result = await warningService.clear(Number(req.params.id), ctx(req))
  res.success(result)
}))

router.put('/:id/sustain', authenticate, requireManager, tryCatch(async (req, res) => {
