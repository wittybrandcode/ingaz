...
router.get('/unread', authenticate, async (req: any, res: any) => {
...
router.put('/preferences/:typeKey', authenticate, validate(updateNotificationPrefSchema), async (req: any, res: any) => {
  res.success(await notificationService.updatePreference(req.user.id, req.params.typeKey, req.body))
})

router.get('/daily-summary', authenticate, async (req: any, res: any) => {
  res.success(await notificationService.dailySummary(req.user.id))
})

router.put('/types/batch', authenticate, async (req: any, res: any) => {
