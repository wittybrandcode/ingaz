...
const SCHEMA_SQL = `
  ...
  CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role_id INTEGER REFERENCES roles(id) ON DELETE SET NULL, avatar TEXT, status TEXT DEFAULT 'active' CHECK(status IN ('active', 'inactive')), credit_score INTEGER DEFAULT 10, frozen_at DATETIME, freeze_reason TEXT, unfrozen_at DATETIME, last_credit_recovery DATETIME, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
  ...
  CREATE TABLE IF NOT EXISTS notifications (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER REFERENCES users(id) ON DELETE CASCADE, title TEXT NOT NULL, message TEXT, type TEXT DEFAULT 'info', read INTEGER DEFAULT 0, related_type TEXT, related_id INTEGER, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
