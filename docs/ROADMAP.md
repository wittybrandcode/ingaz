...
## ✅ ما تم إنجازه (Phase 1–7 + C8)

تم إصلاح **11 حرجة + 18 عالية + 65 متوسطة/منخفضة** — 94 قضية:
- Phase 1–7: 93 قضية (أمان, قاعدة بيانات, معاملات, أخطاء, أداء, معماریة, CI/CD)
- C8: `NotificationService` DI — 5 خدمات (SubtaskService, WarningService, CommentService, ProjectService, TaskService)
...
|--------|---------|--------|---------|
| H22 | `ALLOWED_ORIGINS=*` + `credentials: true` مرفوض من المتصفح | `server/src/index.ts:98` | تغيير `*` إلى origin محدد |
| H20 | `health.ts` يستخدم `db.execute(SELECT 1)` بنص خام | `server/src/routes/health.ts:9` | استخدام Drizzle API |
...
```
الآن:      H22 (CORS) + H20 (health.ts) ← سريعتان
التالي:    H14 (autoRecoverCredit) + H5 (RoleService.delete) + H21 (dailySummary)
بعدها:    C16 (seed-full guard) + H12 (parseMentions batch) + H19 (migrate Pool)
أخيراً:   H18 (test coverage 4-6h) + H17 (component tests 3-5h) + C1 (FK ref 30m)
```
